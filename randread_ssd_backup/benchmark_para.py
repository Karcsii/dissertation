...
for x, num_of_blocks in enumerate(block_size):
	...
	for y, num_of_parallel in enumerate(parallel_tasks):
		time.sleep(10)
		command = "sudo fio --name={name} --ioengine=posixaio --rw=randread --numjobs={parallel} --size=1g --iodepth={parallel} --runtime=60 --time_based --end_fsync=1 --bs={block}k --group_reporting".format(name = name, parallel = num_of_parallel, block = num_of_blocks)
